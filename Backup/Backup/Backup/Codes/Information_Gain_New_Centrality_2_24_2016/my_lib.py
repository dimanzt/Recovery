...
def my_draw(H,file_name):
    ...
    #seamus
    #path='c:\\Users\\'+user_name+'\\Documents\Lab Work\\image_graph_dot\\DotFile' + file_name
            ...
            if H.has_edge(id_source,id_target):
                ...
                for k in keydict:
                    if 'normal' == H.edge[id_source][id_target][k]['type']:
                        ...
                        if(flip_coin(prob_arc)):
                            H.add_edge(id_source,id_target,key=k, status='destroyed',true_status="destroyed",labelfont='gray',color='gray',style='dashed',prob=prob_arc)
                            if edge not in edges_dest_really_dest and edge_reverse not in edges_dest_really_dest:
                                edges_dest_really_dest.append(edge)							
                        else:
                            H.add_edge(id_source,id_target,key=k, status='destroyed',true_status="on",labelfont='gray',color='gray',style='dashed',prob=prob_arc)
...
def generate_demand(H,prob,filename,alfa,seed,path_to_stats,distance_metric,mu_x,mu_y,sigma):
        ...
        if max_flow_residual != 0:
            ...
            if(len(paths)>0 and max_flow_residual>0):
                ...
                if (alfa==0.0):
                    ...
                    demand_assigned=assign_demand_shortest(residualGraph,demand,id_source,id_target,mu_x,mu_y,sigma,file_flags,distance_metric,flag_update_all_paths,temp_feasible)
                """
...
def compute_max_value_of_fixed_flow_from_list_of_couple(H,list_of_couple,prob,alfa,filename,seed,path_to_stats,distance_metric,mu_x,mu_y,sigma):
...
def assign_demand_shortest(residualGraph,demand,id_source,id_target,mu_x,mu_y,sigma,file_flags,distance_metric,flag_update_all_paths,path_feasible):
    ...
    weighted_paths=compute_lenght_paths_in_assign_demand(residualGraph,id_source,id_target,mu_x,mu_y,sigma,distance_metric,flag_update_all_paths,path_feasible)
...
def distance_node(H,node_i,node_j,distance_metric):
    ...
    if distance_metric == "one-hop":
        ...
        if distanza_temp==0.0:
            ...
            distanza_temp=0.1
        print 'broken_capacity distance'
        print distanza_temp
        return distanza_temp
...
def prob_distance_node(H,node_i,node_j,distance_metric,mu_x,mu_y,sigma):
...
def compute_lenght_paths(H,id_source,id_target,mu_x,mu_y,sigma,distance_metric,flag_update_all_paths):
            ...
            if H.has_edge(id_source,id_target):
                ...
                for k in keydict:
                        if H[id_source][id_target][k]['type']=='normal':
                            ...
                            dist = prob_distance_node(G,v,w,distance_metric,mu_x,mu_y,sigma)			
#distance_node(H,id_source,id_target,distance_metric)
                            weight_of_path+=dist
...
def compute_lenght_paths_in_assign_demand(H,id_source,id_target,mu_x,mu_y,sigma,distance_metric,flag_update_all_paths,path_feasible):
    ...
    for path in path_feasible:
        ...
        for i in range(0,(len(path)-1),1):
            ...
            for k in keydict:
                if H[id_source][id_target][k]['type']=='normal':
                    #dist = distance_node(H,id_source,id_target,distance_metric)
                    dist = prob_distance_node(G,id_source,id_target,distance_metric,mu_x,mu_y,sigma)#distance_node(H,id_source,id_target,distance_metric)					
                    weight_of_path+=dist
...
def compute_my_betweeness_3(H,green_edges,distance_metric,mu_x,mu_y,sigma):
    ...
    for edge in green_edges:
        ...
        weighted_paths=compute_lenght_paths(H,id_source,id_target,mu_x,mu_y,sigma,distance_metric,True)
        #print 'Path pesati'
        #print weighted_paths
        array_lenghts=[]
        for key in weighted_paths:
...
def compute_my_betweeness_4(H,green_edges,distance_metric,mu_x,mu_y,sigma):
...
#Ottimizzata
def compute_my_betweeness_4_opt(H,green_edges,distance_metric,mu_x,mu_y,sigma):
    ...
    for edge in green_edges:
        ...
        while(flag_demand_satified==False):
            curr_shortest=my_dijkstra_shortest_path(residual_graph,id_source,id_target,mu_x,mu_y,sigma,distance_metric)
...
def compute_shortest_from_set(residualGraph,paths,distance_metric,mu_x,mu_y,sigma):
    count=0
            ...
            for k in keydict:
                if residualGraph.has_edge(source,target,key=k):
                    if residualGraph[source][target][k]['type']=='normal':
                        ...
                        lenght=prob_distance_node(residualGraph,source,target,distance_metric,mu_x,mu_y,sigma)#distance_node(residualGraph,source,target,distance_metric)
...
def remove_red_edges_nodes(H):
    ...
    for edge in new_graph.edges():
        ...
        if new_graph.has_edge(source,target):
            keydict=H[source][target]
            for k in keydict:
                if H[source][target][k]['color']=='red' or H[target][source][k]['color']=='red':				
                    arc=(source,target,k)
                    arc_reverse=(target,source,k)
                    if arc not in edges_to_remove and arc_reverse not in edges_to_remove:
                        edges_to_remove.append(arc)
...
def get_multiple_shortest_path_no_ramification_to_prune(H,distance_metric,mu_x,mu_y,sigma):
    ...
    for couple in green_edges:
        ...
        if edge in shortest_paths_for_bet:
            ...
            #print shortest_paths
            shortest_path=compute_shortest_from_set(H,shortest_paths,distance_metric,mu_x,mu_y,sigma)
            #print shortest_path
...
def get_multiple_path_no_ramification_to_prune(H,temp_graph_supply,destroyed_graph,distance_metric,mu_x,mu_y,sigma):
    ...
    for couple in green_edges:
        ...
        shortest_path=my_dijkstra_shortest_path(supply_graph,id_source,id_target,mu_x,mu_y,sigma,distance_metric)
...
def get_multiple_path_no_ramification_to_prune_after_split(H,temp_graph_supply,destroyed_graph,distance_metric,couple_split,mu_x,mu_y,sigma):
...
def split(H,temp_graph_supply,id_bc,number_of_split,mu_x,mu_y,sigma):
    ...
    new_bet_dict=compute_my_betweeness_3(H,new_green_edges,assign_demand_shortest,mu_x,mu_y,sigma)
    #print new_bet_dict
                ...
                for elem in range(0,len(sorted_array_of_ratio),1):
                    if (bc_found==False):
                        ...
                        if result_routability_check==True:
                            ...
                            new_bet_dict=compute_my_betweeness_3(H,new_green_edges,distance_metric,mu_x,mu_y,sigma)
                            #print new_bet_dict
                            set_betwenness_from_dict(H,new_bet_dict)
...
def split_by_capacity_path_reservation(H,counter_isp,distance_metric,nodes_recovered,probe_nodes,mu_x,mu_y,sigma):
                                ...
                                if (bc_found==False):
                                    ...
                                    if residualGraph[id_source][id_target][key_edge]['splitted_edge']==True:
                                        ...
                                        if check_if_shortest_changed(residualGraph,edge_split):
                                            ...
                                            shortest_path=compute_shortest_from_set(graph_reservation,shortests_contribute_to_bc,distance_metric,mu_x,mu_y,sigma)
...
def split_by_capacity_path(H,counter_isp,distance_metric,nodes_recovered,probe_nodes,mu_x,mu_y,sigma):
            ...
            if flag_no_couple == True:
                ...
                for elem in range(0,len(sorted_array_of_ratio),1):
                    if (bc_found==False):
                        ...
                        shortest_path=compute_shortest_from_set(residualGraph,shortests_contribute_to_bc,distance_metric,mu_x,mu_y,sigma)
...
def split_by_capacity_path_and_ranking(H,counter_isp,distance_metric,nodes_recovered,type_of_bet,probe_nodes,mu_x,mu_y,sigma):
                ...
                for elem in range(0,len(ranked_paths_of_bc),1):
                    if (bc_found==False):
                        ...
                        if result_routability_check==True:
                            ...
                            #print new_bet_dict
                            #set_betwenness_from_dict(H,new_bet_dict)
                            select_betweeness(H,new_green_edges,distance_metric,type_of_bet,mu_x,mu_y,sigma)
                            my_draw(H,'5-isp-%d-split'%(counter_isp))
...
def split_by_capacity_path_and_ranking_max_split(H,counter_isp,distance_metric,nodes_recovered,nodes_truely_recovered,type_of_bet,probe_nodes,mu_x,mu_y,sigma):
                    ...
                    if (bc_found==False):
                        ...
                        if result_routability_check==True:
                            ...
                            if H.node[id_bc]['true_status']=='destroyed':
                                H.node[id_bc]['status']='repaired'
                                H.node[id_bc]['color']='blue'
                            elif H.node[id_bc]['true_status']=='on' and  H.node[id_bc]['status']=='destroyed':
                                H.node[id_bc]['status']='on'
                                H.node[id_bc]['color']='""'								
                            elif H.node[id_bc]['color']!='""':# or H.node[id_bc]['color']!='green':
                                H.node[id_bc]['color']='gray'
...
def split_by_capacity_path_and_demand(H,counter_isp,distance_metric,nodes_recovered,type_of_bet,probe_nodes,mu_x,mu_y,sigma):
...
def pruning_multiple(H,paths_for_pruning,counter_isp,counter_pruning,distance_metric,type_of_bet,mu_x,mu_y,sigma):
    #global all_graph_paths
...
def recover_one_hop_edge_green(H,edges_recovered,nodes_recovered,edges_truely_recovered_isp,nodes_truely_recovered_isp,nodes_really_dest,edges_really_dest):
        ...
        if supply_edge==True:
            ...
            if max_flow_on_residual < demand :
                ...
                if H[source][target][key_to_recover]['true_status']=='destroyed':
                    ...
                    if edge not in edges_truely_recovered_isp:
                        edges_truely_recovered_isp.append(edge)					
                    ...
                    edge=(source,target)
                    if edge not in edges_really_dest:					
                    #seamus add if for color true_status
                        H.add_edge(source,target,key=key_to_recover, type='normal',status='on',true_status='on',labelfont='black',color='black',style='solid')
                    elif edge in edges_really_dest:					
                        H.add_edge(source,target,key=key_to_recover, type='normal',status='repaired',true_status='on',labelfont='blue',color='blue',style='solid')
...
def recovery_algorithm_ranking_path(H,distance_metric,policy_sort):
    ...
    while check_routability(only_graph_destroyed,green_edges)==False:
        ...
        shortest_path=compute_shortest_from_set(H,paths_same_value,distance_metric,mu_x,mu_y,sigma)
...
def compute_lenght_of_path(H,path,mu_x,mu_y,sigma):
    ...
    for i in range(0,(len(path)-1),1):
        ...
        for k in keydict:
            if H[id_source][id_target][k]['type']=='normal':
                #dist = distance_node(H,id_source,id_target,distance_metric)
                dist = prob_distance_node(H,id_source,id_target,distance_metric,mu_x,mu_y,sigma)#distance_node(H,id_source,id_target,distance_metric)
                weight_of_path+=dist
...
def select_betweeness(H,green_edges,distance_metric,type_of_bet,mu_x,mu_y,sigma):

    if type_of_bet=='classic':
        compute_my_betweeness_1(H,green_edges,distance_metric)

    elif type_of_bet=='aproximate':
        compute_my_betweeness_3(H,green_edges,distance_metric,mu_x,mu_y,sigma)
    elif type_of_bet=='exact':
        return compute_my_betweeness_4_opt(H,green_edges,distance_metric,mu_x,mu_y,sigma)
    elif type_of_bet=='diman':
        return compute_my_betweeness_5_opt(H,green_edges,distance_metric,mu_x,mu_y,sigma)		
    else:
...
#Diman added:
def my_prob_dijkstra_shortest_path(H,source,target,mu_x,mu_y,sigma,distance_metric):

    (length, path) = my_prob_single_source_dijkstra(H, mu_x,mu_y,sigma,distance_metric, source=source, target=target)
    #print length
...
def my_dijkstra_shortest_path(H,source,target,mu_x,mu_y,sigma,distance_metric):

    (length, path) = my_single_source_dijkstra(mu_x,mu_y,sigma,distance_metric, H, source=source, target=target)
    #print length
...
def my_prob_single_source_dijkstra(G,mu_x,mu_y,sigma,distance_metric, source, target=None, cutoff=None, weight='weight'):
    ...
    while fringe:
        ...
        for w, edgedata in edata:
            ...
            vw_dist = dist[v] + prob_distance_node(G,v,w,distance_metric,mu_x,mu_y,sigma) #+G.node[w][weight]
...
def my_single_source_dijkstra(mu_x,mu_y,sigma,distance_metric,G, source, target=None, cutoff=None, weight='weight'):
    ...
    while fringe:
        ...
        for w, edgedata in edata:
            ...
            vw_dist = dist[v] + prob_distance_node(G,v,w,distance_metric,mu_x,mu_y,sigma)# distance_node(G,v,w,'broken_capacity') #+G.node[w][weight]
...
def resolve_one_hop_nodes(H, G, owned_nodes,discovered_nodes):
    on_nodes = []
    destroyed_nodes = []
        ...
        for node in H.nodes():
            if H.node[node]['id']==id_nodo:
                if id_nodo not in graph_built.nodes():
                    ...
                    #graph_built.node[node]['true_status'] = 'repaired'

def recover_gray(H,nodes_repaired,edge_repaired):
...
def Discover_neighbors(H, G, probe_nodes, k, nodes_really_dest ,edges_really_dest,discovered_edges,discovered_nodes,nodes_recovered_isp,edges_recovered_isp,counter_isp):
    ...
    on_nodes,destroyed_nodes=resolve_one_hop_nodes(H, G, probe_nodes,discovered_nodes)
    #reveal_edges_status(H,G,probe_nodes, nodes_really_dest ,edges_really_dest,discovered_edges,discovered_nodes,nodes_recovered_isp,edges_recovered_isp)
