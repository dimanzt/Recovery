...
def my_draw(H,file_name):
    ...
    #seamus
    #path='c:\\Users\\'+user_name+'\\LabWork\\image_graph_dot\\DotFile' + file_name
    path='C:/Users/user/Novella/image_graph_dot/DotFile/' + file_name
	#print path
...
#DIMAN DEFINED THIIS FUNCTION:	
def destroy_gray_area(H, prob):
    nodes_dest,nodes_grey=destroy_gray_nodes(H,prob)
    edges_dest,edges_grey=destroy_gray_edges(H,prob)

    return nodes_dest,edges_dest,nodes_grey,edges_grey
	
def destroy_gray_nodes(H, prob):
    destroyed_nodes=[]
    greyed_nodes=[]
    ...
    for i in H.nodes():
        ...
        if flip_coin(prob):
            ...
            H.node[i]['status']='normal'
            H.node[i]['color']='gray'
        if id_node not in greyed_nodes:
            greyed_nodes.append(id_node)
            #print 'nodo distrutto:' +str(i)		
    return destroyed_nodes,greyed_nodes	


##############COMMENT THIS


def destroy_gray_edges(H,prob):
                ...
                if(flip_coin(prob)):
                    ...
                    for k in keydict:
                        if 'green'!= H.edge[id_source][id_target][k]['type']:
                            H.add_edge(id_source,id_target,key=k, status='destroyed',labelfont='gray',color='gray',style='dashed')
                    ...
                    for k in keydict:
                        if 'green'!= H.edge[id_source][id_target][k]['type']:
                            H.add_edge(id_source,id_target,key=k, status='normal',labelfont='gray',color='gray',style='bold')
                            edge=(id_source,id_target)
                            edge_reverse=(id_target,id_source)
                if edge not in greyed_edges and edge_reverse not in greyed_edges:
                    greyed_edges.append(edge)
                            #print 'arco distrutto: '+str(id_source)+str(id_target)				

    return destroyed_edges,greyed_edges
##############COMMENT THIS	
	
	
	
def destroy_graph_gray(H,mu_x,mu_y,sigma,prob):

    nodes_dest,nodes_grey=destroy_nodes_gray(H,mu_x,mu_y,sigma,prob)
    edges_dest,edges_grey=destroy_edges_gray(H,mu_x,mu_y,sigma,prob)
...
def destroy_nodes_gray(H,mu_x,mu_y,sigma,prob):
    ...
    return destroyed_nodes,grey_nodes
...
def destroy_edges_gray(H,mu_x,mu_y,sigma,prob_arc):
                    ...
                    for k in keydict:
                        if 'green'!= H.edge[id_source][id_target][k]['type']:
                            if(flip_coin(prob_arc)):
                                ...
                                H.add_edge(id_source,id_target,key=k, status='on',labelfont='gray',color='gray',style='dashed')
                            edge=(id_source,id_target)
                            edge_reverse=(id_target,id_source)
                            if edge not in destroyed_edges and edge_reverse not in destroyed_edges:
                                destroyed_edges.append(edge)
    ...
    return destroyed_edges,grey_edges
...
#Diman
def write_gray_graph(nodes_grey,edges_grey,filename_graph,path_to_stats):
    path_to_file=path_to_stats+filename_graph+'_Gray.txt'
    print path_to_file
    #if not os.path.exists(path_to_file):
    file=open(path_to_file,'w')

    for node in nodes_grey:
        file.write(str(node)+'\n')

    file.write('stop\n')

    for edge in edges_grey:
        edge_str='('+str(edge[0])+','+str(edge[1])+')'
        #print edge_str
        file.write(edge_str+'\n')
...
#Diman
def get_graph_from_greyed_graph(H):
    ...
    for edge in new_graph.edges():
        ...
        if new_graph.has_edge(source,target):
            keydict=H[source][target]
            for k in keydict:
                if H[source][target][k]['type']!='gray': #and H[source][target][k]['status']=='destroyed':
                    arc=(source,target,k)
                    arc_reverse=(target,source,k)
                    if arc not in edges_to_remove and arc_reverse not in edges_to_remove:
                        edges_to_remove.append(arc)
...
def split_by_capacity_path_and_ranking_max_split(H,counter_isp,distance_metric,nodes_recovered,type_of_bet):
...
def resolve_onwed_node_edges(H,G, owned_nodes):
    for node in owned_nodes:
        ...
        for edge in edges:
            ...
            keydict=H[id_source][id_target]
            for k in keydict:
                if G.has_edge(id_source,id_target) and G[id_source][id_target][k]['type'] == 'normal' and G[id_source][id_target][k]['status'] == 'repaired':
                    continue
                if H[id_source][id_target][k]['color']=='gray':
                    cap = H[id_source][id_target][k]['capacity']
                    if H[id_source][id_target][k]['status'] == 'on':
                        if G.has_edge(id_source,id_target):
                            ...
                            G.add_edge(id_source,id_target,type='normal',status='on',capacity=cap,color='black',style='solid')
                    if H[id_source][id_target][k]['status'] == 'destroyed':
                        if G.has_edge(id_source,id_target):
                            G[id_source][id_target][k]['color'] = 'red'
                            G[id_source][id_target][k]['style'] = 'dashed'
                        else:
                            G.add_edge(id_source,id_target,type='normal',status='destroyed',capacity=cap,color='red',style='dashed')

                   #status='on',labelfont='gray',color='gray',style='dashed'

def resolve_one_hop_nodes(H, owned_nodes):
    for node in owned_nodes:
        edges = get_node_edges(H, node)
        for edge in edges:
            id_source=edge[0]
            ...
            if id_target == 9:
                ...
                print H.node[id_target]
            for k in keydict:
                print H[id_source][id_target][k]
                if H[id_source][id_target][k]['color'] == 'green':
                    continue
                if H[id_source][id_target][k]['color'] == 'black' or H[id_source][id_target][k]['color'] == 'blue':
                    if H.node[id_target]['color']=='gray':
                        if H.node[id_target]['status'] == 'on':
                            H.node[id_target]['color'] = '""'
                        elif H.node[id_target]['status'] == 'repaired':
                            H.node[id_target]['color'] = 'blue'
                        else:
                            H.node[id_target]['color'] = 'red'
