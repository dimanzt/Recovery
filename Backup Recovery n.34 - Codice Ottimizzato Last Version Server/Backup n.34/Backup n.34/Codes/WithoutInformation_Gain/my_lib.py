...
def recover_gray_Diman(H,nodes_recovered_isp,edges_recovered_isp,nodes_really_dest, edges_really_dest):
...
def get_k_hop_nodes(H, node_list, n):
    if not node_list:
        return []
    ...
    x = 0
    tmp = []
    while ((x < n and not infinity_flag) or (tmp != k_hop_nodes and infinity_flag)):
...
def information_gain(H, G, owned_nodes, k):
    ...
    tmp = get_k_hop_nodes(H, owned_nodes, k-1)  
    resolve_one_hop_nodes(H, G, tmp)
