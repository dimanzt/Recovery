...
path_to_dot_dir='C:/Users/user/Novella/image_graph_dot/DotFile/'
path_to_image_dir='C:/Users/user/Novella/image_graph_dot/immagini_generate/'
path_to_image_store='C:/Users/user/Novella/image_graph_dot/store_images/'
path_to_stats='C:/Users/user/Novella/image_graph_dot/stats/statistiche/'
path_to_file_simulation='C:/Users/user/Novella/image_graph_dot/current_simulation.txt'
path_to_stat_prog='C:/Users/user/Novella/image_graph_dot/stats/progress_iteration/'
path_to_stat_times='C:/Users/user/Novella/image_graph_dot/stats/times/'
...
print 'Dimensioni del Grafo'
print "Nodi: %d"%H.number_of_nodes()
print "Archi: %d"%H.number_of_edges()
print "Totale: %d"%(H.number_of_nodes()+H.number_of_edges())
#sys.exit(0)
...
print 'seed utilizzato: %f: '%seed_random


print 'Diametro del Grafo: %d'%(compute_diameter_of_graph(H))

print 'Variazione del flusso assegnato: %s'%(flow_c_fixed)
if flow_c_fixed=='False':
    print 'Quantita di flusso da asseganre %d:'%(flow_c_value)
else:
    print 'Vario il numero di coppie fissando il flusso %d'%(flow_c_value)
    print 'Numero di coppie da selezionare: %d'%(number_of_couple)

if fixed_distruption=='True':
    print 'Distruzione fissa'
    #var_distruption=0
else:
    print 'Distruzione che varia'
    print 'Varianza della distruzione: %d '%(var_distruption)
...
print 'Subset of couples Random:'
...
#modifica il grafo assegnando valori casuali alle capacita degli archi
...
#--------Calcola max valore di raggio per la distruzione totale del grafo
"""
...
#GENERA DOMANDA FEASIBLE SU GRAFO SUPPLY
#prob_edge=0.05
#probabilita arco verde passata per argomento
prob_edge=float(prob_edge_passed)

#valore di flusso in proporzione ad alfa (mettere alfa=0.0 se si vuole la domanda casuale tra 0 e maxflow
#alfa=0.70
alfa=float(alpha_passed)
#selezionar la metrica di distanza da adottare per calcolare la lunghezza di un path: 'capacity', 'one-hop', 'broken'
...
if feasible_solution==False:
   ...
   # write_stat_time_simulation(path_to_stat_times,filename_graph,int(sys.argv[5]),int(sys.argv[4]),seed_passed,number_of_couple,0)
    sys.exit('Seed Scartato soluzione infeasible')
...
print 'coppie di archi verdi selezionati:'
...
#casual destroy
#nodes_destroyed,edges_destroyed=destroy_graph(H,29,-95,8) #per abilene
...
#nodes_destroyed,edges_destroyed=destroy_graph(H,300,300,300) #per BellCanada
#Diman
nodes_destroyed,edges_destroyed,nodes_grey,edges_grey=destroy_gray_area(H,0.5)
#nodes_destroyed,edges_destroyed=destroy_all_graph(H)
#nodes_destroyed,edges_destroyed=destroy_gray_area(H, 0.5)
#nodes_destroyed,edges_destroyed=destroy_graph(H,39,-75,10) #per multigraph.gml
...
while ( check_routability(graph_built,copy_of_green_edges)==False  ):
    ...
    while(pruning_flag==True):
        ...
        if recovered_one_hop_flag:
            add_edges_recovered_to_graph(H,graph_built,recovered_edges_one_hop)
    ...
    couple_selected,couples_to_prune,flag_no_split,bc=split_by_capacity_path_and_ranking_max_split(temp_graph_supply,counter_isp,distance_metric,nodes_recovered_isp,type_of_bet_passed)
    ...
    if bc!=None:
        add_node_to_graph_recovered(H,graph_built,bc)
...
recover(H,nodes_recovered_isp,edges_recovered_isp)
...
write_stat_num_reparation(path_to_stats,filename_stat,prob_edge,seed_random,alfa,
                          num_rip_isp_nodes,num_rip_isp_edges,        #ISP
                          num_rip_optimal_nodes,num_rip_optimal_edges,#OPTIMAL
                          num_rip_all_nodes,num_rip_all_edges,       #Multicommodity generale
