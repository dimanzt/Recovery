...
def check_if_are_green(H):
    ...
    for node_source in H.nodes():
        ...
        if count==0:
            ...
            elif  H.node[node_source]['status']=='unknown':
                H.node[node_source]['color']='grey'
...
def destroy_gray_area(H, prob):
    nodes_dest=destroy_gray_nodes(H,prob)
    edges_dest=destroy_gray_edges(H,prob)
...
def write_progress_algorithm_on_file(path_stat_prog,name_graph,name_algo,num_couple,curr_sim,num_tot_simu,curr_seed,num_nodes_repaired,num_edges_repaired,num_max_nodes_repaired,num_max_edges_repaired):
    ...
    file_prog=open(path_stat_prog+name_graph+'_'+str(num_couple)+'_couples_progress.txt','a')
