...
def optimal_expected_random_recovery(H,green_edges):
    ...
    for i in H.nodes():
        ...
        if H.node[i]['color']=='gray':
            node_cost=H.node[i]['prob']+random.uniform(-0.1,0.1)
        ...
        for k in keydict:
            if H[id_source][id_target][k]['type']=='normal' and H[id_source][id_target][k]['type']!='green':
                ...
                if H[id_source][id_target][k]['color']=='gray':
                    edge_cost=H[id_source][id_target][k]['prob']+random.uniform(-0.1,0.1)
