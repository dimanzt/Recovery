...
def optimize(nodes,demand_flows,arcs,capacity,vertex_cost,arc_cost,inflow):
    ...
    for i,j in arcs:
        ...
        usedArc[i,j] = m.addVar(lb=0, obj=arc_cost[i,j], vtype=GRB.CONTINUOUS, name='usedArc_%s_%s' % (i,j))

        #usedArc[j,i] = m.addVar(ub=1, obj=arc_cost[i,j], vtype=GRB.BINARY, name='usedArc_%s_%s' % (j,i))
        usedArc[j,i] = m.addVar(lb=0, obj=arc_cost[i,j], vtype=GRB.CONTINUOUS, name='usedArc_%s_%s' % (j,i))
    ...
    for i in nodes:
        ...
        usedVertex[i] = m.addVar(lb=0, obj=vertex_cost[i], vtype=GRB.CONTINUOUS, name='usedVertex_%s' % (i))
