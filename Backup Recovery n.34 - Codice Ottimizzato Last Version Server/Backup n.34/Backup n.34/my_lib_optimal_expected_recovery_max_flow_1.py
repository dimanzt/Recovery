...
def optimal_expected_recovery_max_flow(H,green_edges):
...
def optimize(nodes,demand_flows,arcs,capacity,vertex_cost,arc_cost,inflow):
    ...
    for i,j in arcs:
        ...
        usedArc[i,j] = m.addVar(ub=1, obj=arc_cost[i,j], vtype=GRB.CONTINUOUS, name='usedArc_%s_%s' % (i,j))

        #usedArc[j,i] = m.addVar(ub=1, obj=arc_cost[i,j], vtype=GRB.BINARY, name='usedArc_%s_%s' % (j,i))
        usedArc[j,i] = m.addVar(ub=1, obj=arc_cost[i,j], vtype=GRB.CONTINUOUS, name='usedArc_%s_%s' % (j,i))
    ...
    for i in nodes:
        ...
        usedVertex[i] = m.addVar(ub=1, obj=vertex_cost[i], vtype=GRB.CONTINUOUS, name='usedVertex_%s' % (i))
          ...
          if var_reference[i]>0:
            if vertex_cost[i] !=0:
              ...
              if temp_max_flow > max_flow:
                ...
                print max_flow
              if temp_max_flow == max_flow:
                if H.node[i]['prob']>= H.node[temp_vertex]['prob']:
                  max_flow=temp_max_flow
                  temp_used_vertex = []
                  temp_vertex = i
                  temp_used_vertex.append(i)
